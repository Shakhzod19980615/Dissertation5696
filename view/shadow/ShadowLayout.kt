package ru.arzonpay.android.ui.view.shadow
...
import ru.arzonpay.android.ui.util.ALPHA_OPAQUE_INT
import ru.surfstudio.android.custom.view.shadow.setPadding
...
open class ShadowLayout @JvmOverloads constructor(
    ...
    init {
        obtainAttrs(context, attrs, defStyleAttr, defStyleRes)
        initView()
    }
    ...
    private fun obtainAttrs(
        ...
        shadowClippedToPadding = typedArray.getBoolean(
            R.styleable.ShadowLayout_shadowClippedToPadding,
            DEFAULT_SHADOW_CLIP_TO_PADDING
        )
        shadowBlurRadius =
            typedArray.getDimensionPixelSize(R.styleable.ShadowLayout_shadowRadius, DEFAULT_RADIUS)
        ...
        if (typedArray.hasValue(R.styleable.ShadowLayout_shadowColor)) {
            shadowColor =
                typedArray.getColor(R.styleable.ShadowLayout_shadowColor, Color.TRANSPARENT)
        }
        typedArray.recycle()
    }
    ...
    override fun dispatchDraw(canvas: Canvas?) {
        if (shouldDrawShadow && shadowBitmap == null && !isInEditMode && width != 0 && height != 0) {
    ...
    private fun createShadowBitmap(sourceBitmap: Bitmap): Bitmap {
        ...
        val widthScale: Float =
            (shadowWidth - shadowLeftOffset - shadowRightOffset) / shadowWidth.toFloat()
